...
{
	if (state) {
		setbit(UPS_BATTERY_DISCHARGE_PORT.OUT, UPS_BATTERY_DISCHARGE_PIN);
	} else {
		clrbit(UPS_BATTERY_DISCHARGE_PORT.OUT, UPS_BATTERY_DISCHARGE_PIN);
	}
	ups->discharge = state;
}
...
{
	if (state) {
		clrbit(UPS_BATTARY_TEMPERATURE_PORT.OUT, UPS_BATTERY_TEMPERATURE_PIN);
		setbit(UPS_BATTARY_TEMPERATURE_PORT.DIR, UPS_BATTERY_TEMPERATURE_PIN);
	} else {
		clrbit(UPS_BATTARY_TEMPERATURE_PORT.DIR, UPS_BATTERY_TEMPERATURE_PIN);
	}
